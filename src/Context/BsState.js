...
const BsState = (props) => {
  ...
  const handleGetLastBooking = async () => {
    ...
    //updating the most recent booking information obtained from the backend.
    setLastBookingDetails(data.data);
  };

  useEffect(() => {
    //getting movies, times, and seats from localstorage and updating the status (helpful when the page loads)
